...
int example12(VL53LMZ_Configuration* p_dev)
{
	...
	int32_t cnh_data_size;            /* This will be used to record the actual size of CNH data 		*/
	...
	status = vl53lmz_cnh_create_agg_map(&cnh_config, 16,                /* Resolution. Must match value used in vl53lmz_set_resolution() */
		0,                /* StartX 		*/
		0,                /* StartY 		*/
		1,                /* MergeX		*/
		1,                /* MergeY		*/
	...
	/* from sensor to host.																*/
	status = vl53lmz_cnh_calc_required_memory(&cnh_config, (uint32_t)&cnh_data_size);
	if (status != VL53LMZ_STATUS_OK)
	{
	...
	while (loop < 10)
	{
		...
		if (isReady)
		{
			...
			for (agg_id = 0; agg_id < cnh_config.nb_of_aggregates; agg_id++)
			{
				...
				/* locations for us.																*/
				vl53lmz_cnh_get_block_addresses(&cnh_config, agg_id, cnh_data_buffer, &(p_hist), &(p_hist_scaler), &(p_ambient), &(p_ambient_scaler));
