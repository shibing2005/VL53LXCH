...
int example4(VL53LMZ_Configuration *p_dev)
{
	 ...
	 */
	status = vl53lmz_set_power_mode(p_dev, VL53LMZ_POWER_MODE_SLEEP);
	if(status)
	{
	...
	status = vl53lmz_set_power_mode(p_dev, VL53LMZ_POWER_MODE_WAKEUP);
	if(status)
	{
		printf("vl53lmz_set_power_mode failed, status %u\n", status);
		return status;
